...
recursive function admit (n,m,omega_k,fa1,fa2,fa3,fv1,fv2,fv3,rho,mu,&
                          r_root,r_min,Lr,q,g,asym,expo,lrr_A,lrr_V) result (Y)
    ...
    real(lng)    :: alpha, beta, nu, r, r_d, l_A, l_V, A, A_d, D_A, D_V, wom, F0_A, F0_V, &
                        diameter, eta_D, C_exp, Hct_term,Hct,mu1,mu_eff
    integer :: j,k
...
!    mu_eff  = (1.0 + (eta_D - 1.0)*Hct_term*(diameter/(diameter - 1.1))**2.0)*(diameter/(diameter - 1.1))**2.0
!    mu1 = mu*mu_eff/3.2
!    nu = mu1/rho ! Kinematic blood viscosity,cm^2/s.
...
! write(*, *) 'rho - ', rho
    ...
    if (kappa_A == 0) then
        F0_A    = (pi*rho*g*Lr*(r**4))/(8*mu1*l_A*q)
!       F0_A    = pi*(r**4)/(8*mu1*l_A)
        YA(1,1) =  F0_A
        YA(2,1) = -F0_A
        YA(1,2) = -F0_A
        YA(2,2) =  F0_A
...
!       Fk_A    = i*g_omega/(q*sin(kappa_A))
    ...
    if (kappa_V == 0) then
        F0_V    = (pi*rho*g*Lr*(r**4))/(8*mu1*l_V*q)
!       F0_V    = pi*(r**4)/(8*mu1*l_V)
        YV(1,1) =  F0_V
        YV(2,1) = -F0_V
        YV(1,2) = -F0_V
        YV(2,2) =  F0_V
...
!       Fk_V    = i*g_omega/(q*sin(kappa_V))
    ...
    if (r < r_min) then
        ...
        if (done(n+1,m) == 1) then
            ...
            d1 = admit(n+1,m,omega_k,fa1,fa2,fa3,fv1,fv2,fv3,rho,mu1,r_root,r_min,Lr,q,g,asym,expo,lrr_A,lrr_V)
        end if

    if (done(n,m+1) == 1) then
        d2 = comp(n,m+1,:,:)
    else
        d2 = admit(n,m+1,omega_k,fa1,fa2,fa3,fv1,fv2,fv3,rho,mu1,r_root,r_min,Lr,q,g,asym,expo,lrr_A,lrr_V)
    end if
...
function series (YA, YB) result(Y)
    ...
    Y(1,1) =  (DA + YA(1,1) * YB(1,1)) / E
    Y(1,2) = -(     YA(1,2) * YB(1,2)) / E
    Y(2,1) = -(     YA(2,1) * YB(2,1)) / E
    Y(2,2) =  (DB + YA(2,2) * YB(2,2)) / E

end function series

subroutine impedance (tmstps,Per,rho,mu1,r_root,r_min,y11,y12,y21,y22, &
...
implicit none
  ...
  real(lng), intent(in)      :: Per,rho,mu1,Lr,q,g,r_root,r_min,fa1,fa2,fa3,fv1,fv2,fv3,asym,expo,lrrA,lrrv
...
! write(*, *) 'rho - ', rho ! Check rho is being passed correctly
!write(*,*)'impedance is called'


! Physical parameters
df     = 1/Per                               ! Frequency interval.
...
do k = tmstps/2+1, tmstps+1
  ...
  comp = 0.0_lng
  Y(k-1,:,:) = admit(0,0,Omega(k),fa1,fa2,fa3,fv1,fv2,fv3,rho,mu1,r_root,r_min,Lr,q,g,asym,expo,lrrA,lrrV)
end do
...
y11 = real(IFFT(bitreverse(FFTshift(YHAT11/Per))),lng)
y12 = real(IFFT(bitreverse(FFTshift(YHAT12/Per))),lng)
y21 = real(IFFT(bitreverse(FFTshift(YHAT21/Per))),lng)
y22 = real(IFFT(bitreverse(FFTshift(YHAT22/Per))),lng)



open(unit=1, file='OriginalAdmit',status='replace')
do k = 1, tmstps
