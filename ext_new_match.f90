...
function series (YA, YB) result(Y)
    ...
    Y(1,1) = -((YA(1,2)*YA(2,1)+YA(1,2)*YA(2,2))/(YA(2,2)+YB(1,1))-YA(1,1)-YA(1,2))
    Y(1,2) = -((YA(1,2)*YB(1,1)+YA(1,2)*YB(1,2))/(YA(2,2)+YB(1,1)))
    Y(2,1) = -((YB(2,1)*YA(2,1)+YB(2,1)*YA(2,2))/(YB(2,2)+YA(1,1)))
    Y(2,2) = -((YB(2,1)*YB(1,1)+YB(2,1)*YB(1,2))/(YB(2,2)+YA(1,1))-YB(2,1)-YB(2,2)) ! error revised by YJ

end function series


subroutine external (tmstps,Per,rho,mu,r_root,r_min,yE11,yE12,yE21,yE22, &
                      Lr,q,g,fa1,fa2,fa3,fv1,fv2,fv3,asym,expo,lrrA,lrrV)

implicit none

  integer,   intent(in)      :: tmstps
  real(lng), intent(in)      :: Per,rho,mu,Lr,q,g,r_root,r_min,fa1,fa2,fa3,fv1,fv2,fv3,asym,expo,lrrA,lrrv
  real(lng)                  :: yE11(tmstps),yE12(tmstps),yE21(tmstps),yE22(tmstps)
...
do k = tmstps/2+1, tmstps+1
  ...
  comp = 0.0_lng
  Y(k-1,:,:) = admit(0,0,Omega(k),fa1,fa2,fa3,fv1,fv2,fv3,rho,mu,r_root,r_min,Lr,q,g,asym,expo,lrrA,lrrV)
end do
!write(*,*) 'r_root - ', r_root
...
yE11 = real(IFFT(bitreverse(FFTshift(YHAT11/Per))),lng)
yE12 = real(IFFT(bitreverse(FFTshift(YHAT12/Per))),lng)
yE21 = real(IFFT(bitreverse(FFTshift(YHAT21/Per))),lng)
yE22 = real(IFFT(bitreverse(FFTshift(YHAT22/Per))),lng)



open(unit=1, file='ExternalAdmit',status='replace')
do k = 1, tmstps
write(1,12) Omega(k)/Lr**3*q, yE11(k)*rho*g*Lr/q, yE12(k)*rho*g*Lr/q, yE21(k)*rho*g*Lr/q, yE22(k)*rho*g*Lr/q
